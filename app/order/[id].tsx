...
import { useCart, CartItem, ReorderIssue } from '@/contexts/CartContext';
import { mockMenuItems, mockVendor } from '@/mocks/vendorData';
import {
...
export default function OrderDetailsScreen() {
  ...
  const { getVendorCart, replaceCart, mergeCart, setActiveVendor } = useCart();
  ...
  const canRate =
    order?.status === 'completed' && order?.orderSource === 'the platform' && !order?.hasRating;
  ...
  const handleReorder = () => {
    ...
    order.items.forEach((orderItem) => {
      ...
      if (!menuItem) {
        issues.push({ type: 'removed', itemName: orderItem.name, reason: 'Item deleted or hidden' });
        return;
      }
      if (!menuItem.inStock) {
        issues.push({ type: 'unavailable', itemName: orderItem.name, reason: 'Out of stock' });
        return;
      }
      const priceChanged = menuItem.price !== orderItem.price;
      if (priceChanged) {
        issues.push({
          type: 'updated',
          itemName: orderItem.name,
          reason: `Price changed from ${formatPriceWithCommas(orderItem.price, getOrderCurrency(order?.vendorId))} to ${formatPriceWithCommas(menuItem.price, getOrderCurrency(order?.vendorId))}`,
        });
      }
      itemsToAdd.push({
        id: menuItem.id,
        ...
        isUpdated: priceChanged,
        requiresSelection: false,
      });
    });
    if (itemsToAdd.length === 0) {
      Alert.alert('No Items Available', 'None of the items from this order are currently available.', [
        { text: 'View Menu', onPress: () => router.push(getVendorStorefrontPath(order.vendorId) as any) },
        { text: 'OK', style: 'cancel' },
      ]);
      return;
    }
    const existingCart = getVendorCart(order.vendorId);
    if (existingCart && existingCart.items.length > 0) {
      setReorderItems(itemsToAdd);
      setReorderIssues(issues);
      setShowMergeModal(true);
    } else {
      setActiveVendor(order.vendorId, order.vendorName);
      replaceCart(order.vendorId, order.vendorName, itemsToAdd);
      router.push({
        pathname: '/cart' as any,
        params: { vendorId: order.vendorId, reorderIssues: JSON.stringify(issues), fromReorder: 'true' },
      });
    }
  };
  ...
  const isVendorAvailable = order.vendorId === 'v1';
  ...
  const showReorder = order.status === 'completed';
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView edges={['bottom']} style={styles.bottomBar}>
        <View style={styles.bottomBarInner}>
          ...
          {showReorder && (
            <TouchableOpacity
              style={[styles.bottomBtn, styles.bottomBtnReorder, !isVendorAvailable && styles.bottomBtnDisabled]}
              onPress={handleReorder}
              activeOpacity={0.8}
              disabled={!isVendorAvailable}
            >
              <RotateCw size={17} color={Colors.white} strokeWidth={2} />
              <Text style={styles.bottomBtnReorderText}>Reorder</Text>
            </TouchableOpacity>
          )}
        </View>
      </SafeAreaView>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>Update your cart?</Text>
            <Text style={styles.modalMessage}>
              You already have items in your cart from {order.vendorName}.
            </Text>
            <TouchableOpacity
              style={styles.modalPrimaryBtn}
              onPress={() => {
                setActiveVendor(order.vendorId, order.vendorName);
                replaceCart(order.vendorId, order.vendorName, reorderItems);
                setShowMergeModal(false);
                router.push({
                  pathname: '/cart' as any,
                  params: { vendorId: order.vendorId, reorderIssues: JSON.stringify(reorderIssues) },
                });
              }}
              activeOpacity={0.8}
            >
              <Text style={styles.modalPrimaryBtnText}>Replace cart</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.modalPrimaryBtn}
              onPress={() => {
                mergeCart(order.vendorId, reorderItems);
                setShowMergeModal(false);
                router.push({
                  pathname: '/cart' as any,
                  params: { vendorId: order.vendorId, reorderIssues: JSON.stringify(reorderIssues) },
              ...
              activeOpacity={0.8}
            >
              <Text style={styles.modalPrimaryBtnText}>Merge items</Text>
            </TouchableOpacity>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  bottomBtnDisabled: { opacity: 0.4, backgroundColor: Colors.border },
  bottomBtnReorderText: { fontSize: 15, fontWeight: '600' as const, color: Colors.white },
  bottomBtnWaiting: {
    flex: 1,
    backgroundColor: Colors.surface,
    borderWidth: 1,
  ...
  modalTitle: { fontSize: 20, fontWeight: '700' as const, color: Colors.text, marginBottom: 8 },
  modalMessage: { fontSize: 15, color: Colors.textMuted, marginBottom: 20, lineHeight: 22 },
  modalPrimaryBtn: {
    backgroundColor: Colors.primary,
    paddingVertical: 14,
    borderRadius: 12,
  ...
  modalPrimaryBtnText: { fontSize: 16, fontWeight: '600' as const, color: Colors.white },
  modalCancelBtn: {
    paddingVertical: 14,
    borderRadius: 12,
    alignItems: 'center' as const,
  ...
  modalCancelText: { fontSize: 16, fontWeight: '600' as const, color: Colors.textSecondary },
