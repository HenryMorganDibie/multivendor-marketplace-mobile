...
export default function OrdersScreen() {
  ...
  const upcomingOrders = useMemo(() => filterUpcomingOrders(orders ?? []), [orders]);
  const pastOrders = useMemo(() => filterPastOrders(orders ?? []), [orders]);
...
const styles = StyleSheet.create({
  ...
  tab: {
    flex: 1, paddingVertical: 16, alignItems: 'center' as const,
    justifyContent: 'center' as const, borderBottomWidth: 2, borderBottomColor: 'transparent',
  },
  tabActive: { borderBottomColor: Colors.text },
  tabText: { fontSize: 16, fontWeight: '500' as const, color: Colors.textSecondary },
