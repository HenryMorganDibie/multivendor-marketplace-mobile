...
import { ChevronLeft, Send, User, Copy, Plus, Package, ChevronRight, Truck, Clock as ClockIcon, MoreVertical, Lock } from 'lucide-react-native';
...
export default function OrderChatScreen() {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            blockUser({
              id: `vendor-${order?.vendorId || orderId}`,
  ...
  const renderMessage = (message: ChatMessage, groupInfo: { isFirstInGroup: boolean; isLastInGroup: boolean } = { isFirstInGroup: true, isLastInGroup: true }) => {
    ...
    const vendorInitial = (vendorName || order?.vendorName || 'V').trim().charAt(0).toUpperCase();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.avatarCircleHeader}>
            <Text style={styles.avatarTextHeader}>{(() => {
              const parts = (vendorName || 'Vendor').trim().split(' ').filter(p => p.length > 0);
          ...
          const items: ChatActionItem[] = [
            {
              ...
              onPress: () => {
                if (order?.vendorId) router.push(`/store/${order.vendorId}` as any);
              },
            },
            {
              id: 'search-chat',
              label: 'Search in chat',
              onPress: () => console.log('[ORDER CHAT] Search in chat'),
            },
            {
              id: 'help-order',
              label: 'Help with this order',
