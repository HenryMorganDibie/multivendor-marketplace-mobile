...
export default function ReceiptBrandingScreen() {
  ...
  const [footerNote, setFooterNote] = useState('Thank you for your order!');
  const [selectedTemplate, setSelectedTemplate] = useState<Template>('classic');
  const [hasLogo, setHasLogo] = useState(false);

  const unsavedChanges = useUnsavedChanges(
    { businessName, businessAddress, footerNote, selectedTemplate, hasLogo },
    false
  );
        ...
        options={{
          ...
          headerRight: () => (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={[receiptHeaderStyles.saveText, !unsavedChanges.hasUnsavedChanges && receiptHeaderStyles.saveTextDisabled]}>Save</Text>
            </TouchableOpacity>
          ),
        }}
      />
...
const receiptHeaderStyles = StyleSheet.create({
  saveText: {
    fontSize: 17,
