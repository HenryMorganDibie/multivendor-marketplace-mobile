...
function getRevenueTrendData(
  ...
  if (all.length === 0) {
    return Array.from({ length: 6 }, (_, i) => {
      ...
      return { label: d.toLocaleDateString('en', { month: 'short' }), value: 0 };
    });
  }
  ...
  return sorted.map(key => {
    ...
    return { label: d.toLocaleDateString('en', { month: 'short', year: '2-digit' }), value: monthMap.get(key) ?? 0 };
  });
}
...
export default function GrowthInsightsScreen() {
  ...
  const sourceMap = useMemo(() => {
    ...
    map['the platform Marketplace'] = {
      ...
      customers: new Set(filteredPlatform.map(o => o.customerName ?? 'unknown')),
    ...
    ALL_EXTERNAL_SOURCES.forEach(src => {
      ...
      map[src] = {
        ...
        customers: new Set(srcOrders.map(o => o.customerName || 'Walk-in')),
        color: cfg.color,
  ...
  const platformMetrics = useMemo(() => {
    ...
    return {
      ...
      totalRevenue: platformRevenue + externalRevenue,
    };
  }, [filteredPlatform, filteredExternal]);
  ...
  const customerMetrics = useMemo(() => {
    ...
    [...filteredPlatform, ...filteredExternal].forEach(o => {
      ...
      allCustomers.set(name, (allCustomers.get(name) ?? 0) + 1);
    });
  ...
  const storefrontMetrics = useMemo(() => {
    const baseSeed = filteredPlatform.length;
    const estimatedVisits = baseSeed * 12 + 47;
    const conversionRate = estimatedVisits > 0
      ? Math.round((filteredPlatform.length / estimatedVisits) * 100)
      : 0;
    return {
  ...
  const revenueTrendData = useMemo(
    () => getRevenueTrendData(filteredPlatform, filteredExternal, timeRange),
    [filteredPlatform, filteredExternal, timeRange]
  );
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView edges={['top']} style={styles.safeTop}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Growth Insights</Text>
          <View style={{ width: 40 }} />
        </View>
      ...
      >
        ...
        {hasAnyData ? (
          <View style={styles.metricsRow}>
            <PrimaryMetricCard
              label="Total Revenue"
              value={formatPriceWithCommas(platformMetrics.totalRevenue, currency)}
              sub="all channels"
                ...
                <View key={src.source} style={styles.sourceRow}>
                  ...
                  <View style={styles.sourceInfo}>
                    ...
                    <Text style={styles.sourceRevenue}>
                      {formatPriceWithCommas(src.revenue, currency)} revenue
                    </Text>
                  </View>
                </View>
              ))}
            ...
            <View style={styles.compareCard}>
              <View style={styles.compareRow}>
                ...
                <View style={styles.compareInfo}>
                  ...
                  <Text style={styles.compareRevenue}>
                    {formatPriceWithCommas(platformMetrics.platformRevenue, currency)}
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.compareRow}>
                ...
                <View style={styles.compareInfo}>
                  ...
                  <Text style={styles.compareRevenue}>
                    {formatPriceWithCommas(platformMetrics.externalRevenue, currency)}
                  </Text>
                </View>
              </View>
        ...
        <View style={styles.section}>
          <SectionHeader
            ...
            subtitle={timeRange === 'all' ? 'All time' : timeRange === '7d' ? 'Last 7 days' : timeRange === '30d' ? 'Last 30 days' : timeRange === '90d' ? 'Last 90 days' : 'All time'}
          />
          {isPro ? (
            <>
                  ...
                  <View key={customer.name}>
                    ...
                    <View style={styles.topCustomerRow}>
                      ...
                      <Text style={styles.topCustomerTotal}>
                        {formatPriceWithCommas(customer.total, currency)}
                      </Text>
                    </View>
                  </View>
                ))}
