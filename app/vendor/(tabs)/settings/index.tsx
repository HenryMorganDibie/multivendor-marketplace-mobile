...
function ProfileCompletionCard({ percent, hasLogo, onPress }: { percent: number; hasLogo: boolean; onPress: () => void }) {
  ...
  const barColor = percent === 100 ? Colors.success : percent >= 60 ? Colors.primary : '#F59E0B';
  ...
  return (
    <TouchableOpacity style={completionCardStyles.container} onPress={onPress} activeOpacity={0.8}>
      <View style={completionCardStyles.topRow}>
        <View style={completionCardStyles.left}>
          <Text style={completionCardStyles.title}>Profile Completion</Text>
          {!hasLogo && (
            <View style={completionCardStyles.alertRow}>
              <AlertCircle size={12} color={Colors.error} />
              <Text style={completionCardStyles.alertText}>Upload a logo to go live</Text>
            </View>
          )}
        </View>
        <Text style={[completionCardStyles.percent, { color: barColor }]}>{percent}%</Text>
      </View>
      <View
        style={completionCardStyles.track}
        onLayout={(e) => setBarWidth(e.nativeEvent.layout.width)}
      >
        <Animated.View
          style={[completionCardStyles.fill, { width: animatedWidth, backgroundColor: barColor }]}
        />
      </View>
      <Text style={completionCardStyles.cta}>Tap to complete your storefront →</Text>
    </TouchableOpacity>
  );
}
...
export default function VendorSettingsScreen() {
  ...
  const { getChatModeLabel } = useVendorChatMode();
  ...
  const isSystemGenerated = plan === 'basic' || username === systemGeneratedUsername;

  const completionPercent = useMemo(() => {
    let pct = 20;
    if (vendor.logoImage) pct += 30;
    if (vendor.bannerImage) pct += 20;
    if (vendor.description?.trim()) pct += 15;
  ...
  const handleItemPress = async (item: string) => {
    ...
    const routeMap: Record<string, string> = {
      ...
      'contact-support': '/vendor/settings/contact-support',
      'report-problem': '/vendor/settings/report-problem',
      'customer-chat-mode': '/vendor/settings/customer-chat-mode',
    };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: getBottomOverlayPadding(insets.bottom) }}>
        <View style={styles.vendorCard}>
          <View style={styles.vendorHeader}>
            <Text style={styles.vendorName}>Sanste Catering</Text>
            <View style={styles.verifiedBadge}>
              <CheckCircle size={14} color={Colors.success} />
              <Text style={styles.verifiedText}>Verified</Text>
            </View>
          </View>
          <Text style={styles.vendorCategory}>Food & Catering</Text>
          <Text style={styles.vendorLocation}>Lagos, Lekki</Text>
          
          <View style={styles.usernameContainer}>
            <Text style={styles.usernameLabel}>@{displayUsername}</Text>
            {isSystemGenerated && plan === 'basic' && (
              <Text style={styles.usernameNote}>System-generated</Text>
            )}
          </View>
          
          <View style={styles.planBadge}>
            <Text style={styles.planText}>Plan: {plan.charAt(0).toUpperCase() + plan.slice(1)}</Text>
          </View>
          <ProfileCompletionCard
            percent={completionPercent}
            hasLogo={!!vendor.logoImage}
            onPress={() => router.push('/vendor/settings/storefront-appearance' as any)}
          />
        </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>ORDERS & MESSAGING</Text>
          <View style={styles.glassCard}>
            {renderSettingsRow('Customer Chat', () => handleItemPress('customer-chat-mode'), true, getChatModeLabel())}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.glassCard}>
            {renderSettingsRow('Invoices & Receipts', () => handleItemPress('invoices'), false)}
            {renderSettingsRow('Reports', () => handleItemPress('reports'), true)}
          </View>
        </View>
