 ...
 * Prefers `customerPublicId` (e.g. STEPHA-17364741), then a formatted order
 * reference. The chat type (Order chat / Inquiry) is rendered as a separate badge.
 */
const getCustomerIdentifierLabel = (item: InboxSnapshot): string | undefined => {
  if (item.customerPublicId) return item.customerPublicId.toUpperCase();
  if (item.conversationType === 'order') {
    ...
    return (
      <View style={styles.itemWrapper}>
        ...
        <OrderConversationListItem
          avatarContent={<CircularAvatar name={displayName} />}
