...
import { Share as ShareIcon, Package, Truck, Check, X, CircleDollarSign, CheckCircle2 } from 'lucide-react-native';
...
import { isOrderCancelled, isOrderRejected, isAwaitingCustomerUpdate } from '@/utils/orderHelpers';
...
import DeclineOrderSheet from '@/features/orders/components/DeclineOrderSheet';
import RequestChangesSheet from '@/features/orders/components/RequestChangesSheet';
  ...
  return (
    <View style={timelineStyles.container}>
      {TIMELINE_STEPS.map((step, index) => {
        ...
        let dotBorderColor: string = Colors.border;
...
export default function VendorOrderDetailsScreen({ orderId, openedFromChat }: Props) {
  ...
  const isAwaitingUpdate = isAwaitingCustomerUpdate(orderStatus);
  ...
  return (
    <View style={styles.container}>
      <OrderHeader
        onBack={() => router.back()}
        onDecline={displayFlags?.showAcceptDecline ? handleDeclineOrder : undefined}
      />
        ...
        <View style={styles.content}>
          ...
          {order.deliveryPreference && (
            <View style={styles.section}>
              ...
              <View style={styles.deliveryPrefCard}>
                {order.deliveryPreference === 'FLEXIBLE' ? (
                  <View style={styles.deliveryPrefRow}>
                    <View style={styles.deliveryPrefBadgeGreen}>
                      <View style={styles.deliveryPrefDot} />
                      <Text style={styles.deliveryPrefBadgeTextGreen}>Flexible</Text>
                    </View>
                    <Text style={styles.deliveryPrefDesc}>Customer is available anytime</Text>
                  </View>
                ) : (
                  <View>
                    <View style={styles.deliveryPrefRow}>
                      <View style={styles.deliveryPrefBadgeOrange}>
                        <View style={styles.deliveryPrefDotOrange} />
                        <Text style={styles.deliveryPrefBadgeTextOrange}>Scheduled</Text>
                      </View>
                    </View>
                    {(order.scheduledDate || order.scheduledTime) && (
                      <View style={styles.deliveryPrefScheduledDetails}>
                        {order.scheduledDate && (
                          <View style={styles.deliveryPrefDetailRow}>
                            <Text style={styles.deliveryPrefDetailLabel}>Date</Text>
                            <Text style={styles.deliveryPrefDetailValue}>{order.scheduledDate}</Text>
                          </View>
                        )}
                        {order.scheduledTime && (
                          <View style={styles.deliveryPrefDetailRow}>
                            <Text style={styles.deliveryPrefDetailLabel}>Time</Text>
                            <Text style={styles.deliveryPrefDetailValue}>{order.scheduledTime}</Text>
                          </View>
                        )}
                      </View>
                    )}
      ...
      {displayFlags && (
        <OrderActionsPanel
          ...
          isAwaitingCustomerUpdate={isAwaitingUpdate}
      ...
      />
      <RequestChangesSheet
        visible={showRequestChangesSheet}
        orderId={order.id}
        orderItems={order.items}
        vendorName={order.vendorName}
        onDismiss={() => setShowRequestChangesSheet(false)}
        onSendRequest={handleSendChangeRequest}
      />
      <DeclineOrderSheet
        visible={showDeclineModal}
        declineReasonCode={declineReasonCode}
        declineReasonText={declineReasonText}
        showDropdown={showDeclineDropdown}
        onSetDeclineReasonCode={setDeclineReasonCode}
        onSetDeclineReasonText={setDeclineReasonText}
        onSetShowDropdown={setShowDeclineDropdown}
        onDismiss={() => setShowDeclineModal(false)}
        onConfirmDecline={confirmDeclineOrder}
      />
      <LaektivaModal
...
const styles = StyleSheet.create({
  ...
  deliveryPrefCard: { backgroundColor: Colors.surface, borderRadius: 16, padding: 16 },
  deliveryPrefRow: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: 10, marginBottom: 4 },
  deliveryPrefBadgeGreen: {
    flexDirection: 'row' as const,
    alignItems: 'center' as const,
    gap: 6,
  ...
  deliveryPrefDot: { width: 7, height: 7, borderRadius: 3.5, backgroundColor: '#16A34A' },
  deliveryPrefBadgeTextGreen: { fontSize: 13, fontWeight: '700' as const, color: '#16A34A' },
  deliveryPrefDesc: { fontSize: 14, color: Colors.textMuted, marginTop: 2 },
  deliveryPrefBadgeOrange: {
    flexDirection: 'row' as const,
    alignItems: 'center' as const,
    gap: 6,
  ...
  deliveryPrefDotOrange: { width: 7, height: 7, borderRadius: 3.5, backgroundColor: '#FF8C42' },
  deliveryPrefBadgeTextOrange: { fontSize: 13, fontWeight: '700' as const, color: '#FF8C42' },
  deliveryPrefScheduledDetails: { marginTop: 12, gap: 8 },
  deliveryPrefDetailRow: {
    flexDirection: 'row' as const,
    justifyContent: 'space-between' as const,
    alignItems: 'center' as const,
  },
  deliveryPrefDetailLabel: { fontSize: 14, color: Colors.textMuted },
  deliveryPrefDetailValue: { fontSize: 14, fontWeight: '600' as const, color: Colors.text },
