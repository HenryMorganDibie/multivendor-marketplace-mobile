...
export const VendorOrderChatScreen = ({ orderId }: Props) => {
  ...
  const [invoicePreviewPaymentStatus, setInvoicePreviewPaymentStatus] = useState<InvoicePaymentStatus>('unpaid');
  const [isSendingInvoice, setIsSendingInvoice] = useState(false);

  const invoicePreviewOrder = useMemo(() => {
    if (!invoicePreviewOrderId) return null;
    return vm.activeOrders.find(o => o.id === invoicePreviewOrderId) || null;
  }, [invoicePreviewOrderId, vm.activeOrders]);

  const combinedMessages = useMemo(() => {
    if (!vm.chat) return [];
    return [...vm.chat.messages, ...localInvoiceMessages];
  }, [vm.chat, localInvoiceMessages]);
  ...
  const [showChangeMethodSheet, setShowChangeMethodSheet] = useState<boolean>(false);
  const activeMethods = useMemo<PaymentMethod[]>(
    () => (vm.vendorId ? getActivePaymentMethods(vm.vendorId) : []),
    [vm.vendorId]
  );
  const paymentProofSubmittedOrConfirmed = useMemo(() => {
    const ps = vm.order?.paymentState;
    return (
      ps === 'CUSTOMER_MARKED_PAID' ||
      ps === 'PAYMENT_UNDER_REVIEW' ||
      ps === 'VENDOR_PAYMENT_CONFIRMED' ||
      ps === 'ORDER_READY' ||
      ps === 'ORDER_COMPLETED'
    );
  }, [vm.order?.paymentState]);
  const canChangeMethod = activeMethods.length >= 2 && !paymentProofSubmittedOrConfirmed;

  const handleOpenChangeMethod = useCallback(() => {
    if (!canChangeMethod) return;
    vm.setShowExistingPaymentModal(false);
    setTimeout(() => setShowChangeMethodSheet(true), 300);
  }, [canChangeMethod, vm]);

  const handleConfirmChangeMethod = useCallback((nextMethod: PaymentMethod) => {
    setShowChangeMethodSheet(false);
    Alert.alert(
      'Send updated payment request?',
      'This will replace the payment instructions for this order going forward. The previous request will remain in the order history.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Send',
          onPress: () => {
            console.log('[VendorChat] Replacing payment method with', nextMethod.id);
            Alert.alert('Payment method updated', 'Updated payment instructions sent to customer.');
          },
        },
      ]
    );
  ...
  const handleSendInvoiceFromPreview = useCallback(async () => {
    if (!invoicePreviewOrder) return;
    setIsSendingInvoice(true);
    try {
      const orderItems = invoicePreviewOrder.items.map(item => ({
        id: `item_${item.id}`,
        name: item.name,
      ...
      const subtotal = invoicePreviewOrder.subtotal || orderItems.reduce((s, i) => s + i.total, 0);
      ...
      console.log('[INVOICE] Invoice created:', invoice.id, '| linked orderId:', invoice.orderId);
      const invoiceMsg: ChatMessage = {
        id: `invoice-msg-${Date.now()}`,
        type: 'invoice',
      ...
      setLocalInvoiceMessages(prev => [...prev, invoiceMsg]);
      setShowInvoicePreviewModal(false);
      setInvoicePreviewOrderId(null);
      console.log('[INVOICE] Invoice created and sent to chat:', invoice.id);
    } catch (err) {
  ...
  }, [invoicePreviewOrder, invoicePreviewPaymentStatus, createInvoice]);

  const handleEditInvoicePreview = useCallback(() => {
    setShowInvoicePreviewModal(false);
    const oid = invoicePreviewOrderId;
    setInvoicePreviewOrderId(null);
    setTimeout(() => {
      if (oid) router.push(`/vendor/invoice/${oid}?chatId=${encodeURIComponent(vm.chatId)}` as any);
    }, 300);
  }, [invoicePreviewOrderId, vm.chatId]);
  ...
  const handleInvoicePress = useCallback(() => {
    ...
    if (vm.activeOrders.length > 1) {
      ...
      setInvoicePreviewOrderId(vm.activeOrders[0].id);
      setInvoicePreviewPaymentStatus('unpaid');
      setTimeout(() => setShowInvoicePreviewModal(true), 300);
    }
  }, [vm]);
  ...
  const handleSelectOrder = useCallback((selectedOrderId: string) => {
    ...
    if (vm.documentType === 'invoice') {
      ...
      setInvoicePreviewOrderId(selectedOrderId);
      setInvoicePreviewPaymentStatus('unpaid');
      setTimeout(() => setShowInvoicePreviewModal(true), 300);
  ...
  return (
    <View style={styles.container} testID="vendor-order-chat-screen">
      ...
      <ChatHeader
        ...
        trustLabel={[vm.headerSubtitle, vm.trustIndicators.joinedLabel].filter(Boolean).join(' · ')}
      />
      ...
      >
        <MessageList
          ...
          onSwipeToReply={vm.handleSwipeToReply}
      ...
      <Modal visible={vm.showCustomerProfile} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => vm.setShowCustomerProfile(false)}>
        <SafeAreaView style={styles.modalContainer} edges={['top', 'bottom']}>
          ...
          <ScrollView style={styles.profileContent} showsVerticalScrollIndicator={false}>
            ...
            <TouchableOpacity
              style={styles.notesCompactRow}
              onPress={vm.handleOpenNotesModal}
              activeOpacity={0.7}
              testID="vendor-notes-row-chat"
            >
              <Text style={styles.notesCompactLabel}>Vendor Notes</Text>
              <View style={styles.notesCompactRight}>
                {vm.customerNoteText ? (
                  <Text style={styles.notesCompactPreview} numberOfLines={1}>
                    {vm.customerNoteText}
                  </Text>
                ) : (
                  <Text style={styles.notesCompactAdd}>Add</Text>
                )}
      ...
      {/* Notes Modal */}
      <Modal visible={vm.showNotesModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={vm.handleCancelNote}>
        <KeyboardAvoidingView
            ...
            {vm.existingPaymentRequest?.paymentRequestData && (
              <>
                ...
                <View style={styles.existingPaymentActions}>
                  ...
                  {canChangeMethod && (
                    <TouchableOpacity style={styles.existingPaymentCopyButton} onPress={handleOpenChangeMethod} activeOpacity={0.7} testID="change-payment-method">
                      <Text style={styles.existingPaymentCopyText}>Change payment method</Text>
                    </TouchableOpacity>
                  )}
                ...
                {paymentProofSubmittedOrConfirmed && activeMethods.length >= 2 && (
                  <Text style={styles.existingPaymentDuplicateNote}>
                    Payment method cannot be changed after proof has been submitted or payment has been confirmed.
                  </Text>
                )}
      ...
      {/* Change Payment Method Sheet */}
      <Modal visible={showChangeMethodSheet} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowChangeMethodSheet(false)}>
        <SafeAreaView style={styles.modalContainer} edges={['top', 'bottom']}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={() => setShowChangeMethodSheet(false)}>
              <Text style={styles.modalCancelText}>Cancel</Text>
            </TouchableOpacity>
          ...
          <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
            {activeMethods.map((m) => (
              <TouchableOpacity
                key={m.id}
                onPress={() => handleConfirmChangeMethod(m)}
                activeOpacity={0.8}
                style={{
                  padding: 14,
                  borderRadius: 12,
              ...
              >
                <Text style={{ fontSize: 15, fontWeight: '600', color: Colors.text }}>{m.label}</Text>
                {m.bankName ? (
                  <Text style={{ fontSize: 13, color: Colors.textSecondary, marginTop: 4 }}>{m.bankName}</Text>
                ) : null}
                {m.maskedIdentifier ? (
                  <Text style={{ fontSize: 13, color: Colors.textSecondary, marginTop: 2 }}>{m.maskedIdentifier}</Text>
                ) : null}
              </TouchableOpacity>
            ))}
          </ScrollView>
      ...
      <Modal
        visible={showInvoicePreviewModal}
        animationType="slide"
        presentationStyle="pageSheet"
        onRequestClose={() => { setShowInvoicePreviewModal(false); setInvoicePreviewOrderId(null); }}
      >
        <SafeAreaView style={styles.modalContainer} edges={['top']}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={() => { setShowInvoicePreviewModal(false); setInvoicePreviewOrderId(null); }}>
              <Text style={styles.modalCancelText}>Cancel</Text>
            </TouchableOpacity>
            <Text style={styles.modalTitle}>Invoice Preview</Text>
            <TouchableOpacity onPress={handleEditInvoicePreview}>
              <Text style={styles.modalActionText}>Edit</Text>
            </TouchableOpacity>
          </View>
          <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
            {invoicePreviewOrder && (
              <View style={styles.invPreviewContent}>
                <View style={styles.invPreviewBlock}>
                  <Text style={styles.invPreviewBlockLabel}>BILLED TO</Text>
                  <Text style={styles.invPreviewCustomerName}>
                    {formatCustomerNameFromFull(invoicePreviewOrder.customerName || 'Customer')}
                  </Text>
                </View>

                <View style={styles.invPreviewBlock}>
                  <Text style={styles.invPreviewBlockLabel}>ITEMS</Text>
                  <View style={styles.invPreviewItemsCard}>
                    {invoicePreviewOrder.items.map((item, index) => (
                      <View
                        key={item.id}
                        style={[
                          styles.invPreviewItemRow,
                          index < invoicePreviewOrder.items.length - 1 && styles.invPreviewItemRowBorder,
                        ]}
                      >
                ...
                <View style={styles.invPreviewTotalsCard}>
                  {invoicePreviewOrder.subtotal > 0 && (
                  ...
                  {invoicePreviewOrder.discount > 0 && (
                    <View style={styles.invPreviewTotalRow}>
                ...
                <View style={styles.invPreviewBlock}>
                  <Text style={styles.invPreviewBlockLabel}>PAYMENT STATUS</Text>
                  <View style={styles.invPreviewStatusRow}>
                    {(['unpaid', 'partially_paid', 'paid'] as InvoicePaymentStatus[]).map((status) => {
                      const cfg = {
                        unpaid: { label: 'Unpaid', color: Colors.textMuted, bg: Colors.surface },
                        partially_paid: { label: 'Partial', color: Colors.warning, bg: Colors.warningLight },
                        paid: { label: 'Paid', color: Colors.success, bg: Colors.successLight },
                      }[status];
                      const sel = invoicePreviewPaymentStatus === status;
                      return (
                        <TouchableOpacity
                          key={status}
                          style={[
                            styles.invPreviewStatusChip,
                            { backgroundColor: sel ? cfg.bg : Colors.surface },
                            sel && { borderColor: cfg.color },
                          ]}
                          onPress={() => setInvoicePreviewPaymentStatus(status)}
                          activeOpacity={0.7}
                        >
                          <Text style={[styles.invPreviewStatusChipText, { color: sel ? cfg.color : Colors.textMuted }]}>
                            {cfg.label}
                          </Text>
                        </TouchableOpacity>
                      );
          ...
          <SafeAreaView edges={['bottom']} style={styles.invPreviewBottomBar}>
            <TouchableOpacity
              style={[styles.invPreviewSendBtn, isSendingInvoice && { opacity: 0.6 }]}
              onPress={handleSendInvoiceFromPreview}
            ...
            >
              {isSendingInvoice ? (
                ...
                <Text style={styles.invPreviewSendBtnText}>Send Invoice</Text>
              )}
            </TouchableOpacity>
          </SafeAreaView>
        </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  blockedBanner: { backgroundColor: Colors.errorLight, paddingHorizontal: 16, paddingVertical: 12 },
  ...
  actionsMenuContainer: { backgroundColor: Colors.white, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingTop: 16, paddingBottom: 32, paddingHorizontal: 16 },
  ...
  actionsMenuIconCircle: { width: 56, height: 56, borderRadius: 28, backgroundColor: Colors.primarySoft, alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ...
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 16, borderBottomWidth: 0.5, borderBottomColor: '#F3F4F6' },
  ...
  quickRepliesHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 16, borderBottomWidth: 0.5, borderBottomColor: '#F3F4F6' },
  ...
  quickReplyItem: { paddingVertical: 16, paddingHorizontal: 16, borderBottomWidth: 0.5, borderBottomColor: '#F3F4F6' },
  ...
  notesCompactRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: Colors.border },
  notesCompactLabel: { fontSize: 15, fontWeight: '500', color: Colors.text },
  notesCompactRight: { flexDirection: 'row', alignItems: 'center', gap: 6, flex: 1, justifyContent: 'flex-end', marginLeft: 12 },
  notesCompactPreview: { fontSize: 14, color: Colors.textSecondary, flex: 1, textAlign: 'right' },
  notesCompactAdd: { fontSize: 14, color: Colors.primary, fontWeight: '500' },
  ...
  orderStatusBadgeCompleted: { backgroundColor: '#2C2C2E' },
  orderStatusBadgeAccepted: { backgroundColor: '#2C2C2E' },
  orderStatusBadgeInProgress: { backgroundColor: '#2C2C2E' },
  orderStatusBadgeCancelled: { backgroundColor: '#2C2C2E' },
  ...
  secureModalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 16, backgroundColor: Colors.surface },
  ...
  notesModalContainer: { flex: 1, backgroundColor: Colors.background, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  notesModalHandle: { width: 36, height: 4, borderRadius: 2, backgroundColor: Colors.border, alignSelf: 'center', marginTop: 10, marginBottom: 4 },
  notesModalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: Colors.border },
  notesModalHeaderButton: { paddingVertical: 4, paddingHorizontal: 4, minWidth: 60 },
  notesModalTitle: { fontSize: 17, fontWeight: '600', color: Colors.text, flex: 1, textAlign: 'center' },
  notesModalCancel: { fontSize: 16, color: Colors.textSecondary },
  notesModalSave: { fontSize: 16, fontWeight: '600', color: Colors.primary, textAlign: 'right' },
  notesModalContent: { flex: 1, padding: 20 },
  notesModalInput: { flex: 1, fontSize: 16, color: Colors.text, textAlignVertical: 'top', lineHeight: 24 },
  notesModalFooter: { paddingHorizontal: 20, paddingBottom: 32, paddingTop: 8 },
  notesModalHelperText: { fontSize: 12, color: Colors.textMuted, textAlign: 'center' },
  ...
  orderSelectionStatusPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, backgroundColor: Colors.border },
  ...
  customOrderPreviewImagePlaceholder: { width: 60, height: 60, borderRadius: 8, backgroundColor: Colors.border, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  invPreviewContent: { paddingHorizontal: 16, paddingTop: 8 },
  invPreviewBlock: { marginBottom: 24 },
  invPreviewBlockLabel: { fontSize: 11, fontWeight: '600' as const, color: Colors.textMuted, letterSpacing: 0.8, marginBottom: 8 },
  invPreviewCustomerName: { fontSize: 18, fontWeight: '700' as const, color: Colors.text },
  invPreviewItemsCard: { backgroundColor: Colors.white, borderRadius: 14, borderWidth: 1, borderColor: Colors.border, overflow: 'hidden' as const },
  invPreviewItemRow: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'space-between' as const, paddingVertical: 13, paddingHorizontal: 14 },
  invPreviewItemRowBorder: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.border },
  invPreviewItemName: { fontSize: 15, color: Colors.text, flex: 1 },
  invPreviewItemPrice: { fontSize: 15, fontWeight: '600' as const, color: Colors.text },
  invPreviewTotalsCard: { backgroundColor: Colors.surface, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: Colors.border, marginBottom: 24 },
  invPreviewTotalRow: { flexDirection: 'row' as const, justifyContent: 'space-between' as const, paddingVertical: 4 },
  invPreviewTotalLabel: { fontSize: 14, color: Colors.textSecondary },
  invPreviewTotalValue: { fontSize: 14, color: Colors.text },
  invPreviewGrandRow: { flexDirection: 'row' as const, justifyContent: 'space-between' as const, paddingTop: 10, marginTop: 6, borderTopWidth: 1, borderTopColor: Colors.border },
  invPreviewGrandLabel: { fontSize: 17, fontWeight: '700' as const, color: Colors.text },
  invPreviewGrandValue: { fontSize: 20, fontWeight: '800' as const, color: Colors.text, letterSpacing: -0.5 },
  invPreviewStatusRow: { flexDirection: 'row' as const, gap: 10 },
  invPreviewStatusChip: { flex: 1, paddingVertical: 11, alignItems: 'center' as const, borderRadius: 12, borderWidth: 1.5, borderColor: Colors.border },
  invPreviewStatusChipText: { fontSize: 13, fontWeight: '600' as const },
  invPreviewBottomBar: { backgroundColor: Colors.white },
  invPreviewSendBtn: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'center' as const, backgroundColor: Colors.primary, marginHorizontal: 16, marginVertical: 12, paddingVertical: 16, borderRadius: 14 },
  invPreviewSendBtnText: { fontSize: 16, fontWeight: '700' as const, color: Colors.white },
});
