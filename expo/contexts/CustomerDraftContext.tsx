...
export function CustomerDraftProvider({ children }: { children: ReactNode }) {
   ...
   * "follow the customer across devices" comment on syncDraftToBackend
   * wasn't actually true: a draft typed on device A never appeared on
   * device B. Per the Phase 3 spec (chatDrafts rules), direct client reads
   * are the intended path here (no dedicated getDrafts callable exists) — a
   * plain owner-scoped Firestore query. Merges rather than replaces so an
   * in-flight local edit is never clobbered by an older remote snapshot.
   */
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => console.error('[CustomerDrafts] chatDrafts listener failed:', error)
    );
    return unsubscribe;
  }, []);
   ...
   * or reinstalling — silently lost every message a customer had half-typed.
  ...
  const syncDraftToBackend = useCallback((chatId: string, text: string) => {
    ...
    remoteSaveTimers.current[chatId] = setTimeout(() => {
      ...
      save({ chatId, content: text }).catch((err) => {
        console.error('[CustomerDrafts] saveChatDraft failed:', err);
      });
  ...
  const clearDraft = useCallback(async (chatId: string) => {
    ...
    if (auth.currentUser) {
      ...
      clear({ chatId }).catch((err) => {
        console.error('[CustomerDrafts] clearChatDraft failed:', err);
      });
    }
  }, []);
