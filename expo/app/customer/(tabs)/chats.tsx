...
import { useCustomerSupportChat } from '@/contexts/CustomerSupportChatContext';
 ...
 * Renders the vendor handle in `@handle` format (Instagram/Threads style)
 * so users can see _who_ they are talking to at a glance. The chat type
 * (Order chat / Inquiry) is rendered as a separate badge.
 * Backend-ready: maps from `InboxSnapshot.vendorSlug | vendorPublicId` with
 * a sanitized fallback derived from `title` when neither is present.
 */
...
export default function CustomerChatsScreen() {
  ...
  const handleSupportPress = () => {
    router.push('/settings/support-chat' as any);
  };
