...
export default function CompleteProfileScreen() {
  ...
  const [lastInitial, setLastInitial] = useState<string>(user?.lastInitial ?? '');
  ...
  // Only ask for fields the auth provider did not already supply.
  // Location is collected by the dedicated country → state → area flow
  // (LocationSelectorModal) after this screen, never here.
  const askName = !hasFirstName;
  ...
  const isFormComplete = !askName || firstName.trim().length >= 2;
  ...
  const handleContinue = async () => {
    ...
    setIsLoading(true);
    console.log('[COMPLETE PROFILE] Saving profile (name only), askName:', askName);

    try {
      await updateUserProfile({
        firstName: trimmed || user?.firstName,
        lastInitial: lastInitial.trim().toUpperCase() || user?.lastInitial,
        lastName: lastInitial.trim().toUpperCase() || user?.lastName,
        onboardingCompleted: true,
      });

      console.log('[COMPLETE PROFILE] Profile saved → redirecting to /customer for dedicated location flow');
            ...
            >
              <View style={styles.header}>
                ...
                <Text style={styles.subtitle}>
                  Just a couple of details to personalize your account.
                </Text>
              </View>
              ...
              <View style={styles.form}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!isFormComplete || isLoading) && styles.continueButtonDisabled,
                  ]}
