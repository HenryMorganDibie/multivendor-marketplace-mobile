...
// =============================================================================
// BACKEND-ALIGNED VERIFICATION & ACCOUNT STATES
// =============================================================================
//
// The backend keeps two SEPARATE concepts that older code mixed into one
// vendorStatus string:
//   - Verification state: progress through KYC/KYB review.
//   - Account state: whether the account itself is usable.
// These helpers expose them independently so screens never conflate the two.
// =============================================================================

/** Backend-aligned vendor verification state. */
...
/** Backend-aligned vendor account state. */
export type VendorAccountState = 'active' | 'suspended' | 'deactivated' | 'frozen';

/**
 * Resolves a vendor's backend-aligned verification state.
 * Prefers the explicit `verificationStatus` field; otherwise derives it from
 * the legacy `vendorStatus` string.
 */
export function getVendorVerificationStatus(vendor: Vendor): VendorVerificationStatus {
  if (vendor.verificationStatus) return vendor.verificationStatus;
...
/**
 * Resolves a vendor's backend-aligned account state.
 * Prefers the explicit `accountState` field; otherwise derives it from
 * the legacy `vendorStatus` string.
 */
export function getVendorAccountState(vendor: Vendor): VendorAccountState {
  if (vendor.accountState) return vendor.accountState;
...
// isVendorBlocked) consume ONLY InternalVerificationStatus. They never read
...
export function canAppearInPublicDiscovery(vendor: Vendor): boolean {
  const isApproved = getVendorVerificationStatus(vendor) === 'approved';
  const isActive = getVendorAccountState(vendor) === 'active';
  // isDiscoverable defaults to true when unset so existing mock vendors remain
  // visible; only an explicit `false` opts a vendor out of public discovery.
  const isDiscoverable = vendor.isDiscoverable !== false;
  return isApproved && isActive && isDiscoverable;
}
...
export function isVendorAccountBlocked(vendor: Vendor): boolean {
  const accountState = getVendorAccountState(vendor);
  return accountState === 'suspended' || accountState === 'deactivated' || accountState === 'frozen';
}
