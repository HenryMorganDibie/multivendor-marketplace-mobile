...
export default function LocationSelectorModal({ visible, onComplete }: LocationSelectorModalProps) {
  ...
  const areas = useMemo(() => {
    ...
    return getAreasByRegion(selectedRegion.id);
  }, [selectedRegion]);
  ...
  const handleAreaSelect = useCallback(async (area: AreaInfo | null) => {
    console.log('[LOCATION_MODAL] Area selected:', area?.name || 'skipped');
    if (!selectedRegion || !selectedCountry) return;
    setIsSubmitting(true);
    try {
      console.log('[LOCATION_MODAL] Finalizing location:', selectedCountry.code, selectedRegion.id, area?.name);
      const success = await setInitialCountry(selectedCountry.code, selectedRegion.id, undefined, area?.name);
      if (success) {
  ...
  }, [selectedRegion, selectedCountry, setInitialCountry, onComplete]);
  ...
  const getStepTitle = () => {
    switch (step) {
      ...
      case 'area': return 'Select your area';
    }
  };
  ...
  const getStepSubtitle = () => {
    switch (step) {
      ...
      case 'area': return `Choose your area in ${selectedRegion?.name} (optional)`;
    }
  };
              ...
              <>
                ...
                <View style={styles.listCard}>
                  {filteredAreas.map((areaItem, index) => (
                    <React.Fragment key={`${areaItem.regionId}-${areaItem.name}`}>
                      <TouchableOpacity
                        style={styles.listRow}
                      ...
                      >
                        ...
                        <View style={styles.listRowContent}>
                          <Text style={styles.listRowTitle}>{areaItem.name}</Text>
                          <Text style={styles.listRowMeta}>{selectedRegion?.name}</Text>
                        </View>
                        <ChevronRight size={18} color={Colors.textMuted} />
                      </TouchableOpacity>
