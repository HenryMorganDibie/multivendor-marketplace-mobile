...
export function selectPaymentDisplayState(
  ...
  const isCustomerMarkedPaid = order.paymentState === 'CUSTOMER_MARKED_PAID';
  const hasPaymentProof = (order.paymentProof?.length ?? 0) > 0;
  ...
  return {
    ...
    isCustomerMarkedPaid,
    customerMarkedPaidAt: order.customerMarkedPaidAt ?? null,
    hasPaymentProof,
    paymentProofCount: order.paymentProof?.length ?? 0,
    vendorPaymentProofRequested: order.vendorPaymentProofRequested ?? false,
    showVendorPaymentActions: isCustomerMarkedPaid && order.paymentState !== 'VENDOR_PAYMENT_CONFIRMED',
  };
}
