...
interface CartItemRowProps {
  ...
  onItemNamePress: (itemId: string) => void;
  isLocked?: boolean;
}
...
export function CartItemRow({
  ...
  const isOnSale = displayPrice < basePrice;

  const addOnTotal =
    item.addOns?.reduce((sum: number, addOn) => sum + addOn.price, 0) || 0;
  const unitPrice = displayPrice + addOnTotal;
  const originalUnitPrice = basePrice + addOnTotal;
  const lineTotal = unitPrice * item.quantity;

  const unitSavings = isOnSale ? (basePrice - displayPrice) : 0;
  ...
  return (
    <View style={styles.swipeableContainer}>
      ...
      >
        <TouchableOpacity
          onPress={() => onItemNamePress(item.id)}
          activeOpacity={0.7}
